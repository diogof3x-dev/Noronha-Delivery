...
export function FeaturedRow({
  ...
  return (
    <ul className="-mx-4 flex gap-3 overflow-x-auto px-4 pb-4 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
      {items.map((it) => {
        const hasPromo =
        ...
        return (
          <li key={it.id} className="w-32 shrink-0">
            <div className="relative aspect-square">
              <div className="relative h-full w-full overflow-hidden rounded-xl bg-secondary">
                {it.imageUrl ? (
                  ...
                  <span className="flex h-full w-full items-center justify-center text-2xl font-bold text-muted-foreground/40">
                    {it.name.charAt(0)}
                  </span>
                )}
                {hasPromo && (
            ...
            <div className="mt-2">
              ...
              <p className="line-clamp-2 text-xs leading-tight">{it.name}</p>
            </div>
          </li>
        );
      })}

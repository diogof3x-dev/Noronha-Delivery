...
import { useCart, type CartBusiness } from "@/lib/cart-store";
...
export function AddToCartButton({ business, item, outOfStock }: Props) {
  ...
  if (existing) {
    return <QuantitySteps serviceId={item.serviceId} quantity={existing.quantity} />;
  }
...
function QuantitySteps({ serviceId, quantity }: { serviceId: string; quantity: number }) {

...
import { CreateRentalItemForm } from "./create-form";

export const dynamic = "force-dynamic";

export default async function PainelEquipamentos() {
  ...
  type Item = {
    ...
    is_active: boolean;
    meta: { deposit_cents?: number } | null;
  };
  ...
  const items: Item[] = bizIds.length
    ? ((
        await supabase
          .from("services")
          .select("id, business_id, name, description, price_cents, image_url, is_active, meta")
    ...
    <div className="space-y-6 p-4 md:p-8">
      <header>
        ...
        <h1 className="mt-1 text-2xl font-bold tracking-tight md:text-3xl">
          {isAdmin ? "Equipamentos da plataforma" : "Seus equipamentos"}
        </h1>
        <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
          Cadastre bikes, scooters, buggies, pranchas, máscaras de mergulho — qualquer item
          que aluga por dia. Reservas verificam overlap automaticamente.
        </p>
      </header>

      {!businesses?.length ? (
        <div className="rounded-2xl border border-dashed border-border bg-card p-10 text-center">
          <h2 className="text-lg font-semibold">Nenhuma locadora cadastrada</h2>
          <p className="mt-2 text-sm text-muted-foreground">
            Crie sua loja em <strong>Minha loja</strong> com tipo <em>Aluguel</em>.
          </p>
        </div>
      ) : (
              ...
              <section key={b.id} className="space-y-3">
                <header>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {list.length} equipamento{list.length === 1 ? "" : "s"}
                  </p>
                </header>
                ...
                {list.length === 0 ? (
                  <div className="flex flex-col items-center gap-3 rounded-2xl border border-dashed border-border bg-card p-8 text-center">
                    ...
                    <p className="text-sm text-muted-foreground">Sem equipamentos ainda.</p>
                  </div>
                ) : (
                  <ul className="space-y-2">
                    {list.map((it) => {
                      const deposit = it.meta?.deposit_cents ?? 0;
                      return (
                        <li
                          key={it.id}
                ...
                <CreateRentalItemForm businessId={b.id} />
              </section>
            );
          })}
        </div>
